        ...
        {crops.map((crop, i) => (
          ...
          >
            ...
            <div className={`absolute inset-0 rounded-2xl blur-xl opacity-0 group-hover:opacity-30 transition-opacity duration-300 ${
              crop.status === 'healthy' ? 'bg-emerald-500' :
              crop.status === 'warning' ? 'bg-yellow-500' :
              'bg-red-500'
            }`} />
            
            {/* Card */}
            <div className={`relative p-6 rounded-2xl border backdrop-blur-sm transition-all duration-300 shadow-xl ${
              crop.status === 'healthy' 
              ...
              {/* Status Badge */}
              <div className={`text-center mb-4 px-3 py-1 rounded-full text-xs inline-flex items-center gap-1 ${
                crop.status === 'healthy' 
              ...
              <div className="mb-4">
                ...
                <div className="h-1.5 rounded-full bg-slate-700 overflow-hidden">
                  <motion.div
                    className={`h-full ${
                      crop.stressLevel < 20 ? 'bg-emerald-500' :
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex gap-3">
            <motion.button
              ...
              whileTap={{ scale: 0.95 }}
              className="px-6 py-3 rounded-lg bg-gradient-to-r from-emerald-500 to-lime-500 text-slate-900 text-sm"
            >
              New Scan
            </motion.button>
            <motion.button
